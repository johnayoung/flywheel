...
**The binding constraint these imply:** a single, verifiable statement of intent that simultaneously binds agents (`Task` + graders) and informs humans (legible status). This binding has two production sides, and flywheel owns working drafts of both. The **write side** — authoring intent an optimizing agent cannot game — ships today as the `fw-*` spec/plan/verify authoring skills (`flywheel init --skills`, documented in [workflow.md](workflow.md)): they interview an author into ungameable, end-state success criteria and compile them into `Task`/`Grader`. This is where bets 1 and 2 are made real — the durable asset being authored is the spec plus its graders, not the code. The **read side** — projecting verified reality back as legible status — ships as `flywheel status --rollup`. What is still missing is not a third surface but the *closure between them*: today the authored criterion, its grader, and its verified receipt are three disconnected artifacts, not one traceable binding.
