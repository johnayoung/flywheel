...
| `ClaudeCliAgent` | `_submit` | Builds the in-container `claude --print ... stream-json` invocation |
...
| `parse_stream_json` / `iteration_result_from_stream` | `_stream` | stream-json → `flywheel_core.IterationResult` adapter |
...
prompt on stdin (`-p -`, dodging `ARG_MAX`); `--dangerously-skip-permissions`
defaults on, since a headless `--print` run has no TTY to approve tool use. The
JSONL stdout is folded into an `IterationResult` by `parse_stream_json` /
`iteration_result_from_stream` (`_stream.py`).
...
Both are load-bearing operator knowledge (spec 00044), documented not silently dropped:

1. **No container hang-watchdog.** The harness heartbeat reads SDK `Message`
   objects; a message-less container invoker cannot feed it. Bound the run with
   `ContainerSubmitStrategy(exec_timeout=...)` instead — a `threading.Timer` kills
   the whole `docker exec`, effective even when the agent holds stdout open silent.
2. **Policy `[sandbox.env]` does not reach the in-container agent.** `agent_env`
