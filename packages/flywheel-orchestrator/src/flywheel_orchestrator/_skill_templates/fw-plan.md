...
**Scope every per-task grader NARROW — never the whole-repo gate.** Point each grader at the task's own test target(s) and, for the holdout, the *specific dependents'* suites — not the project's build-and-test-everything command (the CI gate, the unpathed test runner, a build of the world). That gate reruns the entire suite (and any slow integration fixtures or service containers it spins up) and is paid up to four times per task: the agent may run it before reporting done, the grader runs it, and submit-time re-verify reruns command graders again — dominating wall-clock while the narrow graders already witness the criterion. The whole-repo gate is the **operator's merge-time gate, run once per phase**, not a per-task grader. A task's blast radius is covered by its behavior grader plus its composition holdout, not by re-running everything.
