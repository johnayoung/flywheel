...
:class:`flywheel.store_protocols.AuditRecord` only, so this module stays
...
# Mirrors :mod:`flywheel.audit`'s follow-loop exit predicate so the
...
# Mirrors :data:`flywheel.audit._cli._CLI_ENV_SEEDS` so the session
...
def _short(value: object, limit: int = _COLLAPSED_BODY_LIMIT) -> str:
    ...
    Mirrors :func:`flywheel.workflow._short` so the session transcript
...
def build_default_redactor() -> Redactor:
    ...
    Mirrors the wiring :mod:`flywheel.audit._cli` applies on the operator-
...
class TranscriptTailer:
    ...
    def fetch(self) -> list[TranscriptEntry]:
        ...
        :func:`flywheel.audit._drain` does. Each record passes through
