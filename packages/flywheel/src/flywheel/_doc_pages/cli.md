...
| `validate` | Statically validate every active task's command graders (shell-parse + repo-relative path existence) without running them. Exits non-zero naming each invalid task, else `All N active task(s) valid.` Directory sources only. | `--tasks-dir`, `--policy` (no `--db`) |
